...
export const DecisionNode = memo(({ data, selected }: NodeProps<WorkflowNodeData>) => {
  const nodeColors = getNodeColors(data.performanceLevel, data.status);
  const ringColor = data.performanceLevel === 'exceptional' ? 'ring-green-300' :
                    data.performanceLevel === 'strong' ? 'ring-blue-300' :
                    data.performanceLevel === 'moderate' ? 'ring-purple-300' :
                    data.performanceLevel === 'weak' ? 'ring-orange-300' :
                    'ring-blue-300';
    ...
    <div className="relative cursor-pointer">
      ...
      >
        <div className="-rotate-45 text-center p-4">
          <div className="font-bold text-sm uppercase tracking-wide text-gray-700">
            {data.label}
          </div>
          {data.decision && (
